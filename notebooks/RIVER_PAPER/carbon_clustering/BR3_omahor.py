...
for stn in range(stn_b,stn_e):
    ...
    for day in range(0,len(dayar)):
        ...
        toma = (tdat['model_output']['OmA'][:,ts_y,ts_x])
        #print(toma)
        daily_omahor[day] = find_depth_deepalg(dp,toma,ts_y,ts_x, verbose = False)

        omah = xr.Dataset({'arag_hor':(['t'], daily_omahor)})
        stn_name = './ncs/' +  run_name + '_' + str(stn)  + 'asat_depth_sp' + str(spacing)+ '.nc'
