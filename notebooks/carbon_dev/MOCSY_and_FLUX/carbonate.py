# TJŠJ's copy of carbonate.py, introducing an optional low-salinity calcium value
...
def calc_carbonate(values, TP, TSi, T, S, P, params=['TA', 'TC'],calc=None):
    ...
    Ca, KCa, KAr = CaSolubility(S, TempK, P, calc)
...
def CaSolubility(S, TempK, P, calc):
    ...
    ***********************************************************************
    ...
    if calc is not None:
        print('using calcium values provided')
        Ca = np.maximum(0.02128 / 40.087 * (S / 1.80655),calc)
        print(Ca)
        print('what Ca would be if it was calculated from salinity')
        Ca2 = 0.02128 / 40.087 * (S / 1.80655)
        print(Ca2)
    else:
        print('no calcium values given, Ca linear fxn of salinity')
        print('Ca calculated from salinity')
