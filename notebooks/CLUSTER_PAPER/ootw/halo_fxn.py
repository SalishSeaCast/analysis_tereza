def yearhalo_de(spacing,stn_b,stn_e,year):
    ...
    trace_list = mf.create_physlist(monlist,daylist,noday,year)
    ...
    for stn in range(stn_b,stn_e):
        ...
        stn_name = '/ocean/tjarniko/MEOPAR/NC_hindcast/' +  str(year)+ '/HALO_TS/stn_' + str(stn)  + 'halo_depth_sp' + str(spacing)+ '.nc'
...
def halo_de(nemo,grid,ts_x,ts_y):
    ...
    delt_z = grid.e3t[0,:,ts_y:ts_y+1,ts_x:ts_x+1]
