...
def freshwater_retrieval_4m(stn_b,stn_e,threshold,year):
    ...
    dirst = '/ocean/tjarniko/MEOPAR/NC_hindcast/' + str(year)
    ...
    if year == 2015:
        ...
        noday = 365
    tracelist = mf.create_physlist(monlist,daylist,noday,year)
    
    print(year)
    print(noday)
    
    #load station data:
    stn_nc = '/ocean/tjarniko/MEOPAR/at3/CLUSTER/verze2pt0/ncfiles/stn_data_sp10.nc'
    ...
    bathy = nc.Dataset('/results/nowcast-sys/NEMO-forcing/grid/bathy_meter_SalishSea.nc', 'r')
    print('bathy loaded')   
    #load grid:
    grid = nc.Dataset('/results/nowcast-sys/NEMO-forcing/grid/mesh_mask_SalishSea2.nc')
    ...
    for s in range(stn_b,stn_e):
        ...
        #depths of cells:
        dz = np.array(grid.variables['e3t'])
        dz = np.squeeze(dz)
        dz = dz[:,stn_y,stn_x]
        zeds_to_mult = dz[0:4]
