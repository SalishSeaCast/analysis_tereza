def bio_de(spacing, stn_start, stn_end, year):
    ...
    dirname = '/data/tjarniko/MEOPAR/analysis_tereza/notebooks/CLUSTER_PAPER/CLEAN/NC_HINDCAST/' + str(year) +'/BIO_TS/'
    ...
    for n in range(stn_start,stn_end):
        ...
        for i in range(day_start,day_end):
            ...
            z1 = 0
            z2 = 40
            MYRI_d, MICZ_d, PHY2_d, PHY_d = int_phyt_fxn(grid,nemo,t1,t2,x1,x2,y1,y2,z1,z2)
        ...
        jan = xr.Dataset({'MYRI': (['t','z'],  MYRI_ar),'MICZ': (['t','z'],  MICZ_ar)
         ,'PHY2': (['t','z'],  PHY2_ar),'PHY': (['t','z'],  PHY_ar)})
...
#     vol = ar_z * d_z
    ...
    MYRI_d = l_MYRI.values
    MICZ_d = l_MICZ.values
    PHY2_d = l_PHY2.values
    PHY_d  = l_PHY.values
    
    return MYRI_d, MICZ_d, PHY2_d, PHY_d
